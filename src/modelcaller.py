...
from trbawarepipeline.datacleansing import Cleansing


class SentimentModelCaller(Job):
    ...
    def run(self):
        csvinputpath = self.desc['input']
        csvoutputpath = self.desc['output']
        headerList = ['ID', 'Text']
        text = [] 
        ...
        # text_idx = np.expand_dims(text_idx, axis=0)
        print(text_idx)
        predict = make_prediction(text_idx)    
        print(f"Result: {predict}\n")
        # with open(csvoutputpath, 'w') as csvw:
        fieldnames = ["id","predict","negative probability","positive probability"]
        with open(csvoutputpath, 'w', encoding='UTF8', newline='') as f:
